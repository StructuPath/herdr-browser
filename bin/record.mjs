...
function assertSafeTarget(root, target, kind = "file") {
	...
	if (fs.existsSync(target)) {
		...
		if (kind.includes("JSON") && !stat.isFile())
	...
	if (configDir) {
		...
		if (fs.existsSync(source)) {
			...
			if (!stat.isFile() || stat.isSymbolicLink() || stat.size > 1024)
				throw new Error("run-id config must be a bounded regular file");
...
function start() {
	...
	return withWorkspaceLock(root, workspaceId, () => {
		if (fs.existsSync(paths.pointer))
			throw new Error("a recording is already active for this workspace");
		if (fs.existsSync(path.join(paths.runs, `run-${runId}`)))
		...
		try {
			...
			atomicWriteJson(root, paths.pointer, {
				schema_version: 1,
				run_id: runId,
				workspace_id: workspaceId,
				browser_session: session,
...
function stop() {
	...
	return withWorkspaceLock(root, workspaceId, () => {
		...
		if (
			...
			throw new Error("active recording pointer is invalid");
		...
		if (
			...
			manifest.browser_session !== pointer.browser_session
		)
			throw new Error("active recording state is inconsistent");
		try {
