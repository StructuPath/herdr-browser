...
const runBrowsePane = (env, input) => spawnSync('bash',
  [path.join(repoRoot, 'scripts', 'browse-pane.sh')],
  { env, encoding: 'utf8', input });

test('open navigates via HERDR_PLUGIN_CLICKED_URL and refuses a bad one', () => {
  fs.rmSync(path.join(stateDir, 'pane-id-w9'), { force: true });
  const ok = runScript('open.sh', [], freshEnv({ HERDR_PLUGIN_CLICKED_URL: 'http://localhost:5000' }));
  assert.equal(ok.status, 0, ok.stderr);
  assert.match(log(), /agent-browser --session herdr-ws-w9 open http:\/\/localhost:5000/);
  const bad = runScript('open.sh', [], freshEnv({ HERDR_PLUGIN_CLICKED_URL: 'file:///etc/passwd' }));
...
test('URL policy: userinfo refused, uppercase scheme accepted, path @ kept', () => {
  ...
  assert.equal(runScript('open.sh', ['https://user:pass@localhost/']).status, 2);
  fs.rmSync(path.join(stateDir, 'pane-id-w9'), { force: true });
...
test('garbage HERDR_BROWSER_LOCK_TRIES falls back to the default and still steals', () => {
  fs.rmSync(path.join(stateDir, 'pane-id-w9'), { force: true });
  fs.mkdirSync(path.join(stateDir, 'open-lock-w9'), { recursive: true });
  const r = runScript('open.sh', [], freshEnv({ HERDR_BROWSER_LOCK_TRIES: 'abc' }));
  assert.equal(r.status, 0, r.stderr);
  assert.match(log(), /plugin pane open/);
}, { timeout: 15000 });
...
test('open without herdr on PATH names the real problem', () => {
  ...
  fs.copyFileSync(path.join(stubDir, 'agent-browser'), path.join(bare, 'agent-browser'));
...
test('close warns when the stray-pane sweep is skipped (no node)', () => {
  ...
  fs.rmSync(path.join(stateDir, 'pane-id-w9'), { force: true });
  fs.rmSync(path.join(stateDir, 'browse-ids-w9'), { force: true });
  const r = runScript('close.sh', [], freshEnv({ PATH: `${bare}:/usr/bin:/bin` }));
  assert.equal(r.status, 0, r.stderr);
...
test('browse-pane validates the env URL instead of trusting it', () => {
  for (const bad of ['--user-data-dir=/tmp/x', 'file:///etc/passwd', 'chrome://flags']) {
  ...
  const { env, log: l } = mkBrowsePaneEnv({ HERDR_BROWSE_URL: 'http://localhost:3000' });
  const r = runBrowsePane(env);
  assert.equal(r.status, 0, r.stderr);
